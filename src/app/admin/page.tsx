  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-sm text-fg-secondary">Gerencie o conteúdo do portal.</p>
      </div>
        ...
        {(courses ?? []).length === 0 ? (
          ...
          (courses ?? []).map((course) => (
            <Link key={course.id} href={`/admin/cursos/${course.id}`}>
              <Card padded interactive className="flex items-center justify-between gap-4">
                <div className="flex flex-col">
                  <span className="font-medium text-fg-primary">{course.title}</span>
                  <span className="text-xs text-fg-tertiary">
                    {course.language.toUpperCase()} · {course.level.toUpperCase()} · /{course.slug}
                  </span>
                </div>
                <span
                  className={
